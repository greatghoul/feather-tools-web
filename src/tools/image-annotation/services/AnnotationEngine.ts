...
export function createShape(type, attrs): any {
    switch (type) {
        case 'line':
...
export function createPreviewShape(type, x1, y1, x2, y2, attrs): any {
    let shape;
    const dx = x2 - x1;
    const dy = y2 - y1;
...
export function createFinalShape(type, x1, y1, x2, y2, attrs): any {
    const dx = x2 - x1;
    const dy = y2 - y1;
