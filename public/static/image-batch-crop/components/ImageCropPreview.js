    ...
    return html`
        <div class="mb-4 border-bottom pb-4">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <div>
                    <span class="badge bg-primary me-2">${effectiveCrop.width}×${effectiveCrop.height}</span>
                    <small class="text-muted">${getText('image-batch-crop/result/wheel_hint')}</small>
                </div>
                <button class="btn btn-sm btn-outline-success" onClick=${handleDownload}>
                    <i class="bi bi-download me-1"></i>
                    ${getText('image-batch-crop/result/download')}
                </button>
            </div>
            <div class="bg-light rounded p-2 d-flex justify-content-center">
                <div class=${wrapperStyle} ref=${containerRef}></div>
            </div>
        </div>
    `;
};
