...
import SquareShape from '@/components/SquareShape.js';
import CircleShape from '@/components/CircleShape.js';
import RoundedSquareShape from '@/components/RoundedSquareShape.js';
import TriangleShape from '@/components/TriangleShape.js';
import TransformDataConverter from '@/services/TransformDataConverter.js';
import { getText } from '~/helpers/utils.js';

const shapeSelectorContainerStyle = css`
    .btn {
        white-space: nowrap;
    }
...
const ResultCard = ({ image, onClear }) => {
    ...
    useEffect(() => {
        ...
        img.onerror = () => {
            setError(getText('shape-image/error/load_failed'));
        };
        ...
        if (shapePath) {
            ...
            const captureCroppedImage = (event) => {
                if (stage && shapePath && image) {
                    ...
                    const converter = new TransformDataConverter(updatedShapePath.data(), originalTransform);
                    ...
                    img.onload = () => {
                        ...
                        tempImageLayer.clipFunc((ctx) => {
                            return [new Path2D(updatedPathData, 'evenodd')]
                        });
                        
                        // Set high quality for the output image
                        setCroppedImage(tempImageLayer.toDataURL(originalRect, { quality: 1 }));
    ...
    if (error) {
        return html`
            <div class="card">
                <div class="card-body p-3 text-center text-danger">
                    <p>${error}</p>
                </div>
            </div>
        `;
    ...
    return html`
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">${getText('shape-image/result/title')}</h5>
                <button 
                    class="btn btn-sm btn-outline-danger"
                    onClick=${handleClear}
                    title=${getText('shape-image/result/clear_title')}
                >
                    ${getText('shape-image/result/clear')}
                </button>
            </div>
            <div class="card-body bg-light">
                <div class="d-flex justify-content-center gap-2 shape-selector-container ${shapeSelectorContainerStyle}">
                    ${shapeComponents.map(shape => html`
                        <button 
                            class="btn btn-outline-secondary ${selectedShape === shape.id ? 'active' : ''}"
                            onClick=${() => setSelectedShape(shape.id)}
                            title=${getText(shape.label)}
                        >
                            <${shape.component} />
                        </button>
                    `)}
            ...
            <div class="card-body p-3 text-center">
                <div ref=${canvasRef} class=${canvasContainerStyle}></div>
            </div>
            ${croppedImage ? html`
                <div class="card-footer bg-light text-center">
                    <div class="my-1">
                        <img src=${croppedImage} class="img-fluid image-cropped ${imageCroppedStyle}" />    
                    </div>
                    <button 
                        class="btn btn-primary my-1"
                        onClick=${handleDownload}
                    >
                        ${getText('shape-image/result/download_image')}
                    </button>
                </div>
            ` : null}
        </div>
