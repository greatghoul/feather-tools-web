...
import TasksInputCard from './components/TasksInputCard';
...
const App = () => {
    ...
    const handleCreate = async () => {
        ...
        if (mode === 'tasks' && tasks.length === 0) {
            setResult({ type: 'validation', count: 0, message: t('habitica-batch-tasks/message/no_tasks') });
            return;
        }
        if (mode === 'subtasks') {
            if (!taskTitle.trim()) {
                setResult({ type: 'validation', count: 0, message: t('habitica-batch-tasks/message/no_title') });
                return;
            }
            if (checklist!.length === 0) {
                setResult({ type: 'validation', count: 0, message: t('habitica-batch-tasks/message/no_checklist') });
                return;
            }
        }
        if (!settings.userId.trim() || !settings.apiToken.trim()) {
            setResult({ type: 'validation', count: 0, message: t('habitica-batch-tasks/message/no_credentials') });
            return;
        }
...
<>
        ...
        <div className="habitica-batch-tasks-container">
            <div className="row g-4">
                <div className="col-12">
                    <HabiticaSettingsCard onChange={setSettings} />
                </div>
                <div className="col-12">
                    <TasksInputCard mode={mode} onModeChange={setMode} tasksText={tasksText} onTasksTextChange={setTasksText} taskTitle={taskTitle} onTaskTitleChange={setTaskTitle} checklistText={checklistText} onChecklistTextChange={setChecklistText} type={type} onTypeChange={setType} creating={creating} onCreate={handleCreate} onClear={handleClear} onLoadExample={handleLoadExample} />
                </div>
                <div className="col-12">
