...
} from '../services/AnnotationEngine.js';
import Toolbar from './Toolbar.js';
import AnnotationList from './AnnotationList.js';
import Pixelator from '../../pixelate-images/services/Pixelator.js';
...
const AnnotationCanvas = ({ image }) => {
    ...
    const setupSelectMode = useCallback(() => {
        ...
        if (!annLayer) return;
        annLayer.children.forEach((node) => {
    ...
    const addAnnotation = useCallback((shape) => {
        ...
        if (shape.getAttr('annotationType') === 'mosaic') {
            const redrawAllMosaics = () => {
                ...
                annLayer.children.forEach((child) => {
    ...
    const handleDeleteSelected = useCallback(() => {
        ...
        if (hadMosaic) {
            ...
            if (pxOverlay && srcCanvas && pxLayer) {
                ...
                const srcCtx = srcCanvas.getContext('2d');
                ctx.clearRect(0, 0, pxCanvas.width, pxCanvas.height);
                annotationLayerRef.current.children.forEach((child) => {
                    if (child.getAttr('annotationType') === 'mosaic') {
                        const a = child.getAttrs();
                        ctx.save();
                        ctx.beginPath();
    ...
    return html`
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center py-2">
                <span class="text-truncate fw-medium" title=${image.name}>
                    <i class="bi bi-image me-1"></i>${image.name}
                </span>
                <div class="d-flex gap-1 flex-shrink-0">
                    <button
                        class="btn btn-sm btn-outline-success"
                        onClick=${handleDownload}
                        title=${getText('image-annotation/toolbar/download')}
                    >
                    ...
                    <button
                        ...
                        onClick=${handleCopy}
                        title=${getText('image-annotation/toolbar/copy')}
                    >
                        <i class="bi bi-clipboard"></i>
                    </button>
                </div>
            </div>
            <div class="card-body p-0 d-flex flex-column" style="min-height:400px;max-height:600px;overflow:hidden;">
                <div class="row gx-0 mx-0 flex-grow-1" style="min-height:0;">
                    <${Toolbar}
                        tool=${tool}
                        toolKeys=${toolKeys}
                    ...
                    />
                    <div class="col-12 col-md-8 d-flex justify-content-center align-items-center rounded"
                        style=${{
                            'min-height': '200px',
                            cursor: cursorStyle,
                            position: 'relative',
